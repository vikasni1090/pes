...
export const getFeedbackQualityScores = async (req, res) => {
  try {
    const { examId } = req.params;
    
    // Get completed evaluations for this exam
    const evaluations = await PeerEvaluation.find({ 
      exam: examId, 
      eval_status: 'completed' 
    }).populate('evaluator');
...
for (const evaluation of evaluations) {
      ...
      evaluatorStats[evaluatorId].evaluations.push(eval);
      evaluatorStats[evaluatorId].totalEvaluations++;
    }

    // Get exam details for deadlines
    const exam = await Examination.findById(examId);
    const expectedQuestions = exam.number_of_questions || 5; // Assume 5 questions if not specified

    // Calculate scores for each evaluator
    const reviewers = Object.values(evaluatorStats).map(reviewer => {
      let wordCountTotal = 0;
      let feedbackCompleteness = 0;
      let ratingSpreadTotal = 0;
      let timelinessTotal = 0;

  reviewer.evaluations.forEach(evaluation => {
        // Word count (35%)
        const feedbackText = Array.isArray(evaluation.feedback) ? evaluation.feedback.join(' ') : evaluation.feedback || '';
        const wordCount = feedbackText.trim().split(/\\s+/).filter(Boolean).length;
        wordCountTotal += wordCount;

        // Rating spread (25%) - variance of scores
        if (Array.isArray(evaluation.score) && evaluation.score.length > 1) {
          const mean = eval.score.reduce((a, b) => a + b, 0) / eval.score.length;
          const variance = eval.score.reduce((sum, score) => sum + Math.pow(score - mean, 2), 0) / eval.score.length;
          ratingSpreadTotal += Math.min(1, variance / (expectedQuestions * 10)); // Normalize
        }

        // Completeness (25%) - % of fields filled
        const numFeedbacks = Array.isArray(evaluation.feedback) ? evaluation.feedback.filter(f => f.trim()).length : (evaluation.feedback?.trim() ? 1 : 0);
        const numScores = Array.isArray(eval.score) ? eval.score.filter(s => s != null).length : (eval.score != null ? 1 : 0);
        const completeness = ((numFeedbacks + numScores) / (expectedQuestions * 2)) * 100;
        feedbackCompleteness += Math.min(100, completeness);

        // Timeliness (15%) - days before deadline
        const submissionDate = eval.evaluated_on || new Date();
        const daysEarly = (exam.deadline - submissionDate) / (1000 * 60 * 60 * 24);
        timelinessTotal += Math.min(1, daysEarly / 7); // Normalize to 1 week max
      });

      // Calculate weighted score (0-100)
      const avgWordCountScore = Math.min(50, (wordCountTotal / reviewer.totalEvaluations) / 20 * 100); // 20 words avg target
      const avgRatingSpread = ratingSpreadTotal / reviewer.totalEvaluations * 100;
      const avgCompleteness = feedbackCompleteness / reviewer.totalEvaluations;
      const avgTimeliness = timelinessTotal / reviewer.totalEvaluations * 100;

      const finalScore = (
        avgWordCountScore * 0.35 +
        avgRatingSpread * 0.25 +
      ...
      return {
        ...
        totalEvaluations: reviewer.totalEvaluations,
        breakdown: {
          wordCount: avgWordCountScore,
          ratingSpread: avgRatingSpread,
          completeness: avgCompleteness,
          timeliness: avgTimeliness
        },
        avgWords: Math.round(wordCountTotal / reviewer.totalEvaluations * 10) / 10,
        scoreVariance: ratingSpreadTotal / reviewer.totalEvaluations,
        avgDaysBefore: Math.round(avgTimeliness)
      };
    });
    ...
    console.error('Error calculating feedback quality:', error);
    res.status(500).json({ message: 'Failed to calculate feedback quality scores' });
  }
};
